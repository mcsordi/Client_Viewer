...
export const MenuButtonTool: React.FC<TButtonToll> = ({ text, icon, type }) => {
  const { light } = useContext(themeContext);
  const { focus, setFocus } = useContext(focusTool);

  const [state, dispatch]: any = useReducer(reducer, values);
  ...
  return (
    ...
    ${!focus && `first:bg-amber-400 first:text-white first:hover:text-white`} font-medium hover:border
